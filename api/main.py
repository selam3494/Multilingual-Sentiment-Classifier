# api/main.py
import os
os.environ["TRANSFORMERS_NO_TF"] = "1"  # don't import TF, avoids noisy warnings
...
async def _startup():
    # load once, store on app.state
    model_dir = Path(MODEL_DIR)
    if not model_dir.exists():
        raise RuntimeError(
            f"MODEL_DIR '{MODEL_DIR}' not found. Train first or point MODEL_DIR to a valid folder."
        )
    tok, model, device = load_model(str(model_dir))
    app.state.tok = tok
